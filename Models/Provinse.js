class Provinse{
    ...
    _alt;
    constructor(name){
        const provinse = Provinse.getProvinceByName(name);
        this._short = provinse.short;
        this._name = provinse.name;
        this._country = new Country(provinse.country);
        this._alt = provinse.alt;
    }
    get short() {
        return this._short;
    }
    ...
    static getProvinceByName(name){
        for (let i = 0; i < PROVINSE.length; i++) {
            const provinse = PROVINSE[i];
            if(provinse?.name.toUpperCase() === name.toUpperCase()) 
                return provinse;
        }
    }
}
